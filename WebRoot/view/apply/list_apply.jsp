...
<script type="text/javascript">
	function delteApply(id){
		alert("删除成功");
		var url = baseURL + "/delteApply.do?id=" + id;
	...
	function importExcl(){
		var url = baseURL + "/view/apply/import_apply.jsp"
	...
	function addPage(){
		var url = baseURL + "/view/apply/add_apply.jsp";
		window.location.href = url;
	}
</script>
</head>
...
<div class="container-fluid mimin-wrapper"> 
  ...
  <!-- start: content -->
  <form  action="listApply.do" method="post" id="splitPageForm">
  <div id="content">
    <div class="panel">
      <div class="panel-body">
        <div class="col-md-12">
          <h3 class="animated fadeInLeft">核酸检测申请管理</h3>
        </div>
      </div>
    </div>
      ...
      <div class="col-md-12">
       <div class="panel form-element-padding">
            ...
            <div class="panel-body" style="padding-bottom:30px;" id="searchTable">
			 ...
			 <label class="col-sm-2 control-label text-right">申请标题：</label>
			 <div class="col-sm-2">
			 	<input type="text" id="applyName" name="applyName" value="${searchList.applyName }" 
          	 			class="text_add" style=" width:120px"  />
             </div>
			 <label class="col-sm-2 control-label text-right">申请人：</label>
			 <div class="col-sm-2">
	      ...
	      <div class="col-sm-2">
	      <info:dropDown dictionaryType="applyType" name="applyType"  defaultValue="${searchList.applyType }" 
                    ...
                    <table class="table table-striped table-bordered dataTable no-footer" width="100%" cellspacing="0"  style="width: 100%;">
                      <thead>
                        <tr role="row">
						  ...
						  <th  class="sorting" width="10%">申请标题</th>
						  <th  class="sorting" width="20%">申请内容</th>
						  <th  class="sorting" width="10%">申请人</th>
                      ...
                      <tbody>
                        <c:forEach items="${pagerList.pageList }" varStatus="status" var="listObject" >
					       <tr role="row" class="even">
				        		...
				        		<td class="center">${listObject.applyName }</td>
				        		<td class="center">${listObject.applyContext }</td>
				        		...
				        		<td class="center">${listObject.applyType }</td>
					        	<td class="center">
					         		
					         		<c:if test="${role eq 'user' && listObject.applyType eq '申请中'}">
					         		<a href="<%=baseURL %>/findApply.do?id=${listObject.resourceId}" title="编辑" class="link_icon">编辑</a>
         		|
         		<a href="javascript:delteApply('${listObject.resourceId}');" title="删除" class="link_icon">删除</a>
					        	</c:if>
					        	<c:if test="${role eq 'work' && listObject.applyType eq '申请中'}">
					         		<a href="<%=baseURL %>/findApply.do?id=${listObject.resourceId}&type=author" title="处理" class="link_icon">处理</a>
					        	</c:if>
					        	</td>
					       </tr>
					      </c:forEach>
...
<script type="text/javascript">
function ev_export(){
	var action = baseURL+"/exportApply.do";
    var form = $("<form></form>");
