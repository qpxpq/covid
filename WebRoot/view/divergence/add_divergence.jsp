...
<info:verifyField verifyFile="divergence.xml" />
<script type="text/javascript">
	$(function(){
		$("#addDivergenceButton").click(function(){
			if(!Validator.vd()) return;
			var resourceId = $("#resourceId").val();
			if(resourceId == "" || resourceId == null){
				alert("新增成功");
				$("#addDivergence").attr("action",baseURL + "/addDivergence.do");
			} else {
				alert("更新成功");
				$("#addDivergence").attr("action",baseURL + "/updateDivergence.do");
			}
			$("#addDivergence").submit();
		});
		...
		$("#returnListButton").click(function(){
			window.location.href = baseURL + "/listDivergence.do";
		});
	});
</script>
</head>
...
<div class="container-fluid mimin-wrapper"> 
  ...
  <form method="post"   id="addDivergence" >
  <!-- start: content -->
  <div id="content">
    <div class="panel">
      ...
      <div class="col-md-12 padding-0">
        <div class="col-md-12">
            ...
            <div class="panel-body" style="padding-bottom:30px;">
              <div class="col-md-12">
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">登记标题</label>
	                  <div class="col-sm-10">
	                    <input type="text" name="divergenceName" id="divergenceName" value='${pageEntity.divergenceName }'
				         		placeholder="请输入登记标题"  class="col-xs-10 col-sm-5"  />
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">登记内容</label>
	                  <div class="col-sm-10">
	                    <input type="text" name="divergenceContext" id="divergenceContext" value='${pageEntity.divergenceContext }'
				         		placeholder="请输入登记内容"  class="col-xs-10 col-sm-5"  />
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">登记类型</label>
	                  <div class="col-sm-10">
	                    <info:dropDown  dictionaryType="divergenceType" name="divergenceType" 
        									defaultValue='${pageEntity.divergenceType }' styleClass="height:40px;width:345px;border:solid 1px #ddd;color: rgb(204, 204, 204);"  />
	                  </div>
	                </div>
          ...
          <div class="col-md-12 text-center" style="margin:20px;">
              <button id="addDivergenceButton" class="submit btn btn-danger" type="button">保存</button>
            </div>
        </div>
      </div>
    </div>
