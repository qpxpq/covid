...
<script type="text/javascript">
	function delteLeadership(id){
		alert("删除成功");
		var url = baseURL + "/delteLeadership.do?id=" + id;
	...
	function importExcl(){
		var url = baseURL + "/view/leadership/import_leadership.jsp"
	...
	function addPage(){
		var url = baseURL + "/view/leadership/add_leadership.jsp";
		window.location.href = url;
	}
</script>
</head>
  ...
  <div id="content">
    <div class="panel">
      <div class="panel-body">
        <div class="col-md-12">
          <h3 class="animated fadeInLeft">入庆报备管理</h3>
        </div>
      </div>
    </div>
    <div class="col-md-12 padding-0 form-element">
      <div class="col-md-12">
       <div class="panel form-element-padding">
	      ...
	      <div class="col-sm-2">
	      <info:dropDown dictionaryType="disinfectType" name="healthType"  defaultValue="${searchList.healthType }" 
	         	styleClass="height:40px;width:100px;border:solid 1px #ddd;color: rgb(204, 204, 204);"/>
	  	  </div>
                  ...
                  <c:if test="${role eq 'user' }"> <button type="button" onclick="addPage()" class="submit btn btn-danger">添加</button>
                 </c:if>  </div>
                ...
                <div class="row">
                  <div class="col-sm-12">
                    <table style="font-size: 10px;" class="table table-striped table-bordered dataTable no-footer" width="100%" cellspacing="0"  style="width: 100%;">
                      <thead>
                        <tr role="row">
                          <th class="sorting" width="3%">序号</th>
						  <th  class="sorting" width="4%">健康状态</th>
						  <th  class="sorting" width="6%">事由</th>
						  <th  class="sorting" width="8%">来源地</th>
						  <th  class="sorting" width="8%">目的地</th>
						  <th  class="sorting" width="6%">到达站</th>
						  <th  class="sorting" width="8%">身份证号</th>
						  <th  class="sorting" width="6%">用户名称</th>
						  <th  class="sorting" width="6%">审核人</th>
						  <th  class="sorting" width="9%">回复内容</th>
						  <th  class="sorting" width="5%">状态</th>
						  
						  <th  class="sorting" width="6%">操作</th>
						  <th  class="sorting" width="6%">操作</th>
						  <th  class="sorting" width="6%">操作</th>
						  <th  class="sorting" width="6%">操作</th>
						  <th  class="sorting" width="6%">操作</th>
                      ...
                      <tbody>
                        <c:forEach items="${pagerList.pageList }" varStatus="status" var="listObject" >
					       <tr role="row" class="even">
				        		...
				        		<td class="center">${listObject.userName }
					        	...
					        	<td class="center">
					         	<c:if test="${role eq 'user' && listObject.healthType eq '申请中'}">
					         	
					         	<a href="<%=baseURL %>/findLeadership.do?id=${listObject.resourceId}" title="编辑" class="link_icon">编辑</a>
         						|
         						<a href="javascript:delteLeadership('${listObject.resourceId}');" title="删除" class="link_icon">删除</a>
					        	</c:if>
					        	<c:if test="${role eq 'admin' && listObject.healthType eq '申请中'}">
					         		<a href="<%=baseURL %>/findLeadership.do?id=${listObject.resourceId}&type=author" title="处理" class="link_icon">处理</a>
					         	</c:if>
					         	
					         	<c:if test="${role eq 'admin' }">
					         	<td class="center"><a href="<%=baseURL %>/addNews.do?userId=${listObject.userId}&id=a" title="发送消息" class="link_icon">正常通行</a></td>
				        		<td class="center"><a href="<%=baseURL %>/addNews.do?userId=${listObject.userId}&id=b" title="发送消息" class="link_icon">落地核酸</a></td>
				        		<td class="center"><a href="<%=baseURL %>/addNews.do?userId=${listObject.userId}&id=c" title="发送消息" class="link_icon">居家隔离</a></td>
				        		<td class="center"><a href="<%=baseURL %>/addNews.do?userId=${listObject.userId}&id=d" title="发送消息" class="link_icon">集中隔离</a></td>
					        	</c:if>
...
<script type="text/javascript">
function ev_export(){
	var action = baseURL+"/exportLeadership.do";
    var form = $("<form></form>");
