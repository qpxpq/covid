...
<script type="text/javascript">
	function delteVaccinum(id){
		alert("删除成功");
		var url = baseURL + "/delteVaccinum.do?id=" + id;
	...
	function importExcl(){
		var url = baseURL + "/view/vaccinum/import_vaccinum.jsp"
	...
	function addPage(){
		var url = baseURL + "/view/vaccinum/add_vaccinum.jsp";
		window.location.href = url;
	}
</script>
</head>
                    ...
                    <table class="table table-striped table-bordered dataTable no-footer" width="100%" cellspacing="0"  style="width: 100%;">
                      <thead>
                        <tr role="row">
						  ...
						  <th  class="sorting" width="20%">接种日期</th>
                        ...
                        <c:forEach items="${pagerList.pageList }" varStatus="status" var="listObject" >
					       <tr role="row" class="even">
					        	...
					        	<c:if test="${role eq 'work' }"><td class="center">
					         		<a href="<%=baseURL %>/findVaccinum.do?id=${listObject.resourceId}" title="编辑" class="link_icon">编辑</a>
         		|
         		<a href="javascript:delteVaccinum('${listObject.resourceId}');" title="删除" class="link_icon">删除</a>
					        	</td></c:if>
					       </tr>
					      </c:forEach>
					      <tr>
...
<script type="text/javascript">
function ev_export(){
	var action = baseURL+"/exportVaccinum.do";
    var form = $("<form></form>");
