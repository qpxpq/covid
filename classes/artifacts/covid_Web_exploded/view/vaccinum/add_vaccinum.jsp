...
<info:verifyField verifyFile="vaccinum.xml" />
<script type="text/javascript">
	$(function(){
		$("#addVaccinumButton").click(function(){
			if(!Validator.vd()) return;
			var resourceId = $("#resourceId").val();
			if(resourceId == "" || resourceId == null){
				alert("新增成功");
				$("#addVaccinum").attr("action",baseURL + "/addVaccinum.do");
			} else {
				alert("更新成功");
				$("#addVaccinum").attr("action",baseURL + "/updateVaccinum.do");
			}
			$("#addVaccinum").submit();
		});
		...
		$("#returnListButton").click(function(){
			window.location.href = baseURL + "/listVaccinum.do";
		});
	});
</script>
</head>
...
<div class="container-fluid mimin-wrapper"> 
  ...
  <form method="post"   id="addVaccinum" >
  <!-- start: content -->
  <div id="content">
    <div class="panel">
      ...
      <div class="col-md-12 padding-0">
        <div class="col-md-12">
            ...
            <div class="panel-body" style="padding-bottom:30px;">
              <div class="col-md-12">
					<div class="form-group">
	                  ...
	                  <div class="col-sm-10">
	                    <input type="text" name="vaccinumName" id="vaccinumName" value='${pageEntity.vaccinumName }'
				         		placeholder="请输入疫苗名称"  class="col-xs-10 col-sm-5"  />
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">疫苗描述</label>
	                  <div class="col-sm-10">
	                    <input type="text" name="vaccinumContext" id="vaccinumContext" value='${pageEntity.vaccinumContext }'
				         		placeholder="请输入疫苗描述"  class="col-xs-10 col-sm-5"  />
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">疫苗针次</label>
	                  <div class="col-sm-10">
	                    <info:dropDown  dictionaryType="vaccinumType" name="vaccinumType" 
        									defaultValue='${pageEntity.vaccinumType }' styleClass="height:40px;width:345px;border:solid 1px #ddd;color: rgb(204, 204, 204);"  />
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">接种地点</label>
	                  <div class="col-sm-10">
	                    <input type="text" name="address" id="address" value='${pageEntity.address }'
				         		placeholder="请输入接种地点"  class="col-xs-10 col-sm-5"  />
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">接种日期</label>
	                  <div class="col-sm-10">
	                    <input type="text" name="vaccinumDate" onclick="WdatePicker()" id="vaccinumDate" value='<fmt:formatDate value="${pageEntity.vaccinumDate }" pattern="yyyy-MM-dd HH:mm:ss" />'
						         placeholder="请选择接种日期"  class="col-xs-10 col-sm-5"  />
	                  </div>
	                </div>
          ...
          <div class="col-md-12 text-center" style="margin:20px;">
              <button id="addVaccinumButton" class="submit btn btn-danger" type="button">保存</button>
            </div>
        </div>
      </div>
    </div>
