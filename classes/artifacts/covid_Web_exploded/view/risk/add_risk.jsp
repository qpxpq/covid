...
<info:verifyField verifyFile="risk.xml" />
<script type="text/javascript">
	$(function(){
		$("#addRiskButton").click(function(){
			if(!Validator.vd()) return;
			var resourceId = $("#resourceId").val();
			if(resourceId == "" || resourceId == null){
				alert("新增成功");
				$("#addRisk").attr("action",baseURL + "/addRisk.do");
			} else {
				alert("更新成功");
				$("#addRisk").attr("action",baseURL + "/updateRisk.do");
			}
			$("#addRisk").submit();
		});
		...
		$("#returnListButton").click(function(){
			window.location.href = baseURL + "/listRisk.do";
		});
	});
</script>
</head>
      ...
      <div class="col-md-12 padding-0">
        <div class="col-md-12">
            ...
            <div class="panel-body" style="padding-bottom:30px;">
              <div class="col-md-12">
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">风险地区名称</label>
	                  <div class="col-sm-10">
	                    <input type="text" name="riskName" id="riskName" value='${pageEntity.riskName }'
				         		placeholder="请输入风险地区名称"  class="col-xs-10 col-sm-5"  />
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">风险地区等级</label>
	                  <div class="col-sm-10">
	                    <info:dropDown  dictionaryType="riskType" name="riskType" 
        									defaultValue='${pageEntity.riskType }' styleClass="height:40px;width:345px;border:solid 1px #ddd;color: rgb(204, 204, 204);"  />
	                  </div>
	                </div>
          ...
          <div class="col-md-12 text-center" style="margin:20px;">
              <button id="addRiskButton" class="submit btn btn-danger" type="button">保存</button>
            </div>
        </div>
      </div>
    </div>
