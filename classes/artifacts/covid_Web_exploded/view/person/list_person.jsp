...
<script type="text/javascript">
	function deltePerson(id){
		alert("删除成功");
		var url = baseURL + "/deltePerson.do?id=" + id;
	...
	function importExcl(){
		var url = baseURL + "/view/person/import_person.jsp"
	...
	function addPage(){
		var url = baseURL + "/view/person/add_person.jsp";
		window.location.href = url;
	}
</script>
</head>
...
<div class="container-fluid mimin-wrapper"> 
  ...
  <!-- start: content -->
  <form  action="listPerson.do" method="post" id="splitPageForm">
  <div id="content">
    <div class="panel">
      <div class="panel-body">
        <div class="col-md-12">
          <h3 class="animated fadeInLeft">管理员管理</h3>
        </div>
      </div>
    </div>
       ...
       <div class="panel form-element-padding">
            ...
            <div class="panel-body" style="padding-bottom:30px;" id="searchTable">
                <div class="form-group">
                  ...
                  <div class="col-sm-2">
                  	<input type="text" id="personName" name="personName" value="${searchList.personName }" class="form-control"/>    
                  </div>
                  <div class="col-sm-4">
                    ...
                    <table class="table table-striped table-bordered dataTable no-footer" width="100%" cellspacing="0"  style="width: 100%;">
                      <thead>
                        <tr role="row">
						  ...
						  <th class="sorting" width="10%">登陆名</th>
						  <th class="sorting" width="20%">身份证号</th>
						  <th class="sorting" width="10%">姓名</th>
						  <th class="sorting" width="15%">电话</th>
						  <th class="sorting" width="10%">性别</th>
						  <th class="sorting" width="20%">操作</th>
                        </tr>
                      </thead>
                      <tbody>
                        <c:forEach items="${pagerList.pageList }" varStatus="status" var="person" >
					       <tr role="row" class="even">
					        	<td class="center">${status.index + 1 }</td>
					        	<td class="center">${person.personNo }</td>
					        	<td class="center">${person.cardNo }</td>
					        	<td class="center">${person.personName }</td>
					        	<td class="center">${person.personMobile }</td>
					        	<td class="center">${person.sex }</td>
					        	<td class="center">
					         		<a href="<%=baseURL %>/findPerson.do?id=${person.personId }" title="编辑" class="link_icon">编辑</a>
					         		|
					         		<a href="javascript:deltePerson('${person.personId }');" title="删除" class="link_icon">删除</a>
					        	</td>
					       </tr>
					      </c:forEach>
                      </tbody>
...
<script type="text/javascript">
function ev_export(){
	var action = baseURL+"/exportPerson.do";
    var form = $("<form></form>");
