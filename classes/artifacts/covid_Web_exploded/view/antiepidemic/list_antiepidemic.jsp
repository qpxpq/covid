...
<script type="text/javascript">
	function delteAntiepidemic(id){
		alert("删除成功");
		var url = baseURL + "/delteAntiepidemic.do?id=" + id;
	...
	function importExcl(){
		var url = baseURL + "/view/antiepidemic/import_antiepidemic.jsp"
	...
	function addPage(){
		var url = baseURL + "/view/antiepidemic/add_antiepidemic.jsp";
		window.location.href = url;
	}
</script>
</head>
...
<div class="container-fluid mimin-wrapper"> 
  ...
  <!-- start: content -->
  <form  action="listAntiepidemic.do" method="post" id="splitPageForm">
  <div id="content">
    <div class="panel">
      <div class="panel-body">
        <div class="col-md-12">
          <h3 class="animated fadeInLeft">防疫信息管理</h3>
        </div>
      </div>
    </div>
      ...
      <div class="col-md-12">
       <div class="panel form-element-padding">
            ...
            <div class="panel-body" style="padding-bottom:30px;" id="searchTable">
			 ...
			 <label class="col-sm-4 control-label text-right">防疫信息标题：</label>
			 <div class="col-sm-2">
			 	<input type="text" id="antiepidemicName" name="antiepidemicName" value="${searchList.antiepidemicName }" 
                    ...
                    <table style="font-size: 10px;" class="table table-striped table-bordered dataTable no-footer" width="100%" cellspacing="0"  style="width: 100%;">
                      <thead >
                        <tr role="row">
						  ...
						  <th  class="sorting" width="10%">防疫信息标题</th>
						  <th  class="sorting" width="25%">防疫信息描述</th>
						  <th  class="sorting" width="25%">防疫信息要求</th>
						  <th  class="sorting" width="10%">创建人</th>
						  <th  class="sorting" width="15%">创建时间</th>
						 <c:if test="${role eq 'work' }"> <th class="sorting" width="10%">操作</th>
                       </c:if> </tr>
                      </thead>
                      <tbody>
                        <c:forEach items="${pagerList.pageList }" varStatus="status" var="listObject" >
					       <tr role="row" class="even">
				        		...
				        		<td class="center">${listObject.antiepidemicName }</td>
				        		<td class="center">${listObject.antiepidemicContext }</td>
				        		<td class="center">${listObject.antiepidemicYq }</td>
					        	...
					        	<c:if test="${role eq 'work' }"><td class="center">
					         		<a href="<%=baseURL %>/findAntiepidemic.do?id=${listObject.resourceId}" title="编辑" class="link_icon">编辑</a>
         		|
         		<a href="javascript:delteAntiepidemic('${listObject.resourceId}');" title="删除" class="link_icon">删除</a>
					        	</td></c:if>
					       </tr>
					      </c:forEach>
					      <tr>
...
<script type="text/javascript">
function ev_export(){
	var action = baseURL+"/exportAntiepidemic.do";
    var form = $("<form></form>");
