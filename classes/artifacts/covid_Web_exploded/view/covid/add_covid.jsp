...
<info:verifyField verifyFile="covid.xml" />
<script type="text/javascript">
	$(function(){
		$("#addCovidButton").click(function(){
			if(!Validator.vd()) return;
			var resourceId = $("#resourceId").val();
			if(resourceId == "" || resourceId == null){
				alert("新增成功");
				$("#addCovid").attr("action",baseURL + "/addCovid.do");
			} else {
				alert("更新成功");
				$("#addCovid").attr("action",baseURL + "/updateCovid.do");
			}
			$("#addCovid").submit();
		});
		...
		$("#returnListButton").click(function(){
			window.location.href = baseURL + "/listCovid.do";
		});
	});
</script>
</head>
      ...
      <div class="col-md-12 padding-0">
        <div class="col-md-12">
            ...
            <div class="panel-body" style="padding-bottom:30px;">
              <div class="col-md-12">
					<div class="form-group">
	                  ...
	                  <div class="col-sm-10">
	                    <input type="text" name="covidName" id="covidName" value='${pageEntity.covidName }'
				         		placeholder="请输入标题"  class="col-xs-10 col-sm-5"  />
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">内容</label>
	                  <div class="col-sm-10">
						 <textarea style="height:80px;width:310px;border:solid 1px #ddd;color: rgb(204, 204, 204);" id="covidContext" name="covidContext">${pageEntity.covidContext }</textarea>
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">措施</label>
	                  <div class="col-sm-10">
						 <textarea style="height:80px;width:310px;border:solid 1px #ddd;color: rgb(204, 204, 204);" id="covidSc" name="covidSc">${pageEntity.covidSc }</textarea>
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">地区</label>
	                  <div class="col-sm-10">
	                    <input type="text" name="covidAddress" id="covidAddress" value='${pageEntity.covidAddress }'
				         		placeholder="请输入地区"  class="col-xs-10 col-sm-5"  />
	                  </div>
	                </div>
					<div class="form-group">
	                  <label class="col-sm-2 control-label text-right">等级</label>
	                  <div class="col-sm-10">
	                    <info:dropDown  dictionaryType="covidType" name="covidType" 
        									defaultValue='${pageEntity.covidType }' styleClass="height:40px;width:345px;border:solid 1px #ddd;color: rgb(204, 204, 204);"  />
	                  </div>
	                </div>
          ...
          <div class="col-md-12 text-center" style="margin:20px;">
              <button id="addCovidButton" class="submit btn btn-danger" type="button">保存</button>
            </div>
        </div>
      </div>
    </div>
