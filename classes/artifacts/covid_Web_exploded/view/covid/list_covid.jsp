...
<script type="text/javascript">
	function delteCovid(id){
		alert("删除成功");
		var url = baseURL + "/delteCovid.do?id=" + id;
	...
	function importExcl(){
		var url = baseURL + "/view/covid/import_covid.jsp"
	...
	function addPage(){
		var url = baseURL + "/view/covid/add_covid.jsp";
		window.location.href = url;
	}
</script>
</head>
...
<div class="container-fluid mimin-wrapper"> 
  ...
  <!-- start: content -->
  <form  action="listCovid.do" method="post" id="splitPageForm">
  <div id="content">
    <div class="panel">
      <div class="panel-body">
        <div class="col-md-12">
          <h3 class="animated fadeInLeft">疫情信息管理</h3>
        </div>
      </div>
    </div>
      ...
      <div class="col-md-12">
       <div class="panel form-element-padding">
            ...
            <div class="panel-body" style="padding-bottom:30px;" id="searchTable">
			 ...
			 <div class="col-sm-2">
			 	<input type="text" id="covidName" name="covidName" value="${searchList.covidName }" 
          	 			class="text_add" style=" width:120px"  />
             </div>
	      <label class="col-sm-1 control-label text-right">等级：</label>
	      <div class="col-sm-2">
	      <info:dropDown dictionaryType="covidType" name="covidType"  defaultValue="${searchList.covidType }" 
	         	styleClass="height:40px;width:100px;border:solid 1px #ddd;color: rgb(204, 204, 204);"/>
                  ...
                  <div class="col-sm-4">
                    ...
                    <c:if test="${role eq 'work' }">  <button type="button" onclick="addPage()" class="submit btn btn-danger">添加</button>
                 </c:if> </div>
                    ...
                    <table class="table table-striped table-bordered dataTable no-footer" width="100%" cellspacing="0"  style="width: 100%;">
                      <thead>
                        <tr role="row">
						  ...
						  <th  class="sorting" width="15%">标题</th>
						  <th  class="sorting" width="20%">内容</th>
						  <th  class="sorting" width="20%">措施</th>
						  <th  class="sorting" width="10%">地区</th>
						  <th  class="sorting" width="10%">等级</th>
						  <c:if test="${role eq 'work' }">  <th class="sorting" width="10%">操作</th></c:if>
                        </tr>
                      </thead>
                      <tbody>
                        <c:forEach items="${pagerList.pageList }" varStatus="status" var="listObject" >
					       <tr role="row" class="even">
				        		...
				        		<td class="center">${listObject.covidName }</td>
				        		<td class="center">${listObject.covidContext }</td>
				        		<td class="center">${listObject.covidSc }</td>
				        		<td class="center">${listObject.covidAddress }</td>
				        		<td class="center">${listObject.covidType }</td>
					        	 <c:if test="${role eq 'work' }"> <td class="center">
					         		<a href="<%=baseURL %>/findCovid.do?id=${listObject.resourceId}" title="编辑" class="link_icon">编辑</a>
         		|
         		<a href="javascript:delteCovid('${listObject.resourceId}');" title="删除" class="link_icon">删除</a>
					        	</td></c:if>
					       </tr>
					      </c:forEach>
					      <tr>
...
<script type="text/javascript">
function ev_export(){
	var action = baseURL+"/exportCovid.do";
    var form = $("<form></form>");
