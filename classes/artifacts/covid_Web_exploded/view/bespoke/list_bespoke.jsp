...
<script type="text/javascript">
	function delteBespoke(id){
		alert("删除成功");
		var url = baseURL + "/delteBespoke.do?id=" + id;
	...
	function importExcl(){
		var url = baseURL + "/view/bespoke/import_bespoke.jsp"
	...
	function addPage(){
		var url = baseURL + "/view/bespoke/add_bespoke.jsp";
		window.location.href = url;
	}
</script>
</head>
...
<div class="container-fluid mimin-wrapper"> 
  ...
  <!-- start: content -->
  <form  action="listBespoke.do" method="post" id="splitPageForm">
  <div id="content">
    <div class="panel">
      <div class="panel-body">
        <div class="col-md-12">
          <h3 class="animated fadeInLeft">疫苗预约管理</h3>
        </div>
      </div>
    </div>
    <div class="col-md-12 padding-0 form-element">
      <div class="col-md-12">
       <div class="panel form-element-padding">
	      ...
	      <div class="col-sm-2">
	      <info:dropDown dictionaryType="bespokeType" name="accusationType"  defaultValue="${searchList.accusationType }" 
	         	styleClass="height:40px;width:100px;border:solid 1px #ddd;color: rgb(204, 204, 204);"/>
	  	  </div>
                      ...
                      <tbody>
                        <c:forEach items="${pagerList.pageList }" varStatus="status" var="listObject" >
					       <tr role="row" class="even">
					        	...
					        	<td class="center">
					         		<c:if test="${role eq 'user' && listObject.accusationType eq '预约中'}">
					         		<a href="<%=baseURL %>/findBespoke.do?id=${listObject.resourceId}" title="编辑" class="link_icon">编辑</a>
         		|
         		<a href="javascript:delteBespoke('${listObject.resourceId}');" title="删除" class="link_icon">删除</a>
					        	</c:if>
					        	<c:if test="${role eq 'work' && listObject.accusationType eq '预约中'}">
					         		<a href="<%=baseURL %>/findBespoke.do?id=${listObject.resourceId}&type=author" title="审核" class="link_icon">审核</a>
					        	</c:if>
					        	<c:if test="${role eq 'work' && listObject.accusationType eq '同意'}">
					         		<a href="<%=baseURL %>/findBespoke.do?id=${listObject.resourceId}&type=yjz" title="接种" class="link_icon">接种</a>
					        	</c:if>
					        	</td>
					       </tr>
					      </c:forEach>
...
<script type="text/javascript">
function ev_export(){
	var action = baseURL+"/exportBespoke.do";
    var form = $("<form></form>");
