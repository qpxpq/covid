...
<script type="text/javascript">
	function delteNote(id){
		alert("删除成功");
		var url = baseURL + "/delteNote.do?id=" + id;
	...
	function importExcl(){
		var url = baseURL + "/view/note/import_note.jsp"
	...
	function addPage(){
		var url = baseURL + "/view/note/add_note.jsp";
		window.location.href = url;
	}
</script>
</head>
                    ...
                    <table class="table table-striped table-bordered dataTable no-footer" width="100%" cellspacing="0"  style="width: 100%;">
                      <thead>
                        <tr role="row">
						  ...
						  <th  class="sorting" width="20%">帖子标题</th>
						  <th  class="sorting" width="30%">帖子内容</th>
                      ...
                      <tbody>
                        <c:forEach items="${pagerList.pageList }" varStatus="status" var="listObject" >
					       <tr role="row" class="even">
					        	...
					        	<td class="center">
					         		<c:if test="${role eq 'user' && listObject.creatorId eq userSession.resourceId}">
					         		<a href="<%=baseURL %>/findNote.do?id=${listObject.resourceId}" title="编辑" class="link_icon">编辑</a>
         		|
         		<a href="javascript:delteNote('${listObject.resourceId}');" title="删除" class="link_icon">删除</a>
         		</c:if>
         		<c:if test="${role eq 'work' && listObject.creatorId eq workSession.resourceId}">
					         		<a href="<%=baseURL %>/findNote.do?id=${listObject.resourceId}" title="编辑" class="link_icon">编辑</a>
         		|
         		<a href="javascript:delteNote('${listObject.resourceId}');" title="删除" class="link_icon">删除</a>
         		</c:if>
         		         		<a href="<%=baseURL %>/listAppriseNote.do?noteId=${listObject.resourceId}&noteName=${listObject.noteName }" title="评论" class="link_icon">评论</a>
					        	</td>
					       </tr>
					      </c:forEach>
					      <tr>
...
<script type="text/javascript">
function ev_export(){
	var action = baseURL+"/exportNote.do";
    var form = $("<form></form>");
