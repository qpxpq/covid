...
<script type="text/javascript">
	function delteAccusation(id){
		alert("删除成功");
		var url = baseURL + "/delteAccusation.do?id=" + id;
	...
	function importExcl(){
		var url = baseURL + "/view/accusation/import_accusation.jsp"
	...
	function addPage(){
		var url = baseURL + "/view/accusation/add_accusation.jsp";
		window.location.href = url;
	}
</script>
</head>
  ...
  <div id="content">
    <div class="panel">
      <div class="panel-body">
        <div class="col-md-12">
          <h3 class="animated fadeInLeft">核酸检测管理</h3>
        </div>
      </div>
    </div>
                    ...
                    <table class="table table-striped table-bordered dataTable no-footer" width="100%" cellspacing="0"  style="width: 100%;">
                      <thead>
                        <tr role="row">
						  ...
						  <th  class="sorting" width="10%">手机号</th>
						  <th  class="sorting" width="20%">身份证号</th>
                        ...
                        <c:forEach items="${pagerList.pageList }" varStatus="status" var="listObject" >
					       <tr role="row" class="even">
					        	...
					        	<td class="center">
					         		...
					         		<c:if test="${role eq 'user' && listObject.accusationType eq '审核中'}">
					         		<a href="<%=baseURL %>/findAccusation.do?id=${listObject.resourceId}" title="编辑" class="link_icon">编辑</a>
         		|
         		<a href="javascript:delteAccusation('${listObject.resourceId}');" title="删除" class="link_icon">删除</a>
					        	</c:if>
					        	<c:if test="${role eq 'work' && listObject.accusationType eq '审核中'}">
					         		<a href="<%=baseURL %>/findAccusation.do?id=${listObject.resourceId}&type=author" title="处理" class="link_icon">处理</a>
					        	</c:if>
					        	</td>
					       </tr>
					      </c:forEach>
...
<script type="text/javascript">
function ev_export(){
	var action = baseURL+"/exportAccusation.do";
    var form = $("<form></form>");
